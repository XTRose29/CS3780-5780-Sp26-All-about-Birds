...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-root", type=Path, default=Path("raw_10_per_label"))
    parser.add_argument("--report-out", type=Path, default=Path("raw_10_per_label/sizes.csv"))
...
def save_grayscale_image(spec: np.ndarray, image_path: Path) -> tuple[int, int]:
...
def main() -> None:
    ...
    for index, audio_path in enumerate(audio_files, start=1):
        ...
        size = save_grayscale_image(spec, image_path)
        rows.append(
            {
